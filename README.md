...
| TUI plugin | Loads from `.opencode/tui.json`, adds a right-sidebar `Sandbox Modified Files` list from the real sandbox git diff, watches the worktree's real git `HEAD` for branch changes, shows a sandbox branch badge when OpenCode's built-in branch display points at the original repo, and keeps async polling as a fallback for sandbox file changes. |
