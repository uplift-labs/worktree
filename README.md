...
node .opencode/worktree/core/cmd/worktree-init.ts `
  --repo "$PWD" `
  --session demo `
...
node .opencode/worktree/core/cmd/worktree-lifecycle.ts `
  --repo "$PWD" `
  --worktrees-dir .opencode/worktree/worktrees
```
...
`remote-install.ts` clones the requested release into a temp directory and runs `install.ts`. For testing another branch or tag, pass `--ref <git-ref>` or set `WORKTREE_REF`.
...
OpenCode does not expose a pre-bootstrap hook that mutates its internal project root, so the plugin virtualizes supported tool paths into the worktree. OpenCode's own footer/status may still show the original repo and branch; trust `OPENCODE_WORKTREE_PATH`, tool working dirs, or `git status` run by the tool for active worktree state.

The TUI plugin reads the session worktree from `OPENCODE_WORKTREE_PATH` or the session marker. Tune fallback polling with `AISB_OPENCODE_BRANCH_REFRESH_MS`, `AISB_OPENCODE_FILES_REFRESH_MS`, and `AISB_OPENCODE_GIT_TIMEOUT_MS`. Enable debug logs with `AISB_OPENCODE_BRANCH_DEBUG=1` or `AISB_OPENCODE_FILES_DEBUG=1`.
...
| Command | Purpose |
|---|---|
| `worktree-init.ts` | Create a session worktree branched from `main` or `master`. |
| `worktree-guard.ts` | Path gate: allow or deny an edit based on active worktree location. |
| `worktree-lifecycle.ts` | Cleanup merged worktrees, stale markers, orphan branches, and residual dirs. |
| `worktree-cleanup.ts` | Session cleanup: capture-commit pending work, self-release merged clean marker, then run lifecycle. |
| `worktree-merge-gate.ts` | Pre-merge validation: block if the worktree has uncommitted changes. |
