...
const USAGE = [
  "install.ts - install worktree-sandbox into a target git repo.",
  "",
  "Usage:",
...
export function install(argv: string[]): number {
  ...
  const installRoot = path.join(options.target, options.prefix, "sandbox")
...
function ensureGitignore(target: string, prefix: string): void {
  ...
  const pattern = `/${prefix}/sandbox/worktrees/`
  const current = fs.existsSync(file) ? fs.readFileSync(file, "utf8") : ""
  if (current.split(/\r?\n/).includes(pattern)) return
  fs.appendFileSync(file, `\n# Sandbox worktrees - generated by worktree-sandbox install.ts\n${pattern}\n`, "utf8")
...
const gate = path.join(repoRoot, ${JSON.stringify(prefix)}, "sandbox", "core", "cmd", "sandbox-merge-gate.ts")
