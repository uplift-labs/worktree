...
| OpenCode TUI plugin | Loads from `.opencode/tui.json`, renders a right-sidebar `Sandbox Modified Files` list from the sandbox git diff against the current branch's merge-base with main/master, refreshes branch metadata immediately from git `HEAD` filesystem events when available, shows a `sandbox:<branch>` prompt badge when OpenCode's built-in branch display does not match the sandbox branch, keeps `AISB_OPENCODE_BRANCH_REFRESH_MS` polling as branch fallback, polls sandbox file changes every `2000` ms by default with `AISB_OPENCODE_FILES_REFRESH_MS=0` as the opt-out, and uses async git refreshes with `AISB_OPENCODE_GIT_TIMEOUT_MS` to avoid blocking the OpenCode UI. The prompt branch badge can be disabled with `AISB_OPENCODE_BRANCH_BADGE=0`. The built-in `internal:sidebar-files` deactivation is disabled by default; `AISB_OPENCODE_HIDE_BUILTIN_FILES=1` restores it. |
...
sidebar state and a sandbox branch badge for that mismatch. Any `internal:sidebar-files` deactivation is opt-in runtime-only
